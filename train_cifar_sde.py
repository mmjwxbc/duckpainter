        ...
        for batch in loader:
            ...
            if train_steps % args.log_every == 0:
                ...
                if train_steps % (args.log_every * 10) == 0:
                    with torch.no_grad():
                        model.eval()
                        images = ode_sampler_cfg_label(model, batch_size=16, in_channel=3, null_id=10, guidance_scale=4.0)
                        save_image(images, f"{experiment_dir}/cifra_sample_{train_steps:07d}.png", nrow=4, normalize=True, value_range=(-1, 1))
            # Save DiT checkpoint:
